...
export default function PhraseLearnView({ phrase }: Props) {
  ...
  const tryHref = `/try?q=${encodeURIComponent(phrase.tryPrompt)}`;
  ...
  return (
    <article className="relative min-h-screen overflow-x-hidden bg-[#020617]">
      ...
      <div className="mx-auto w-full max-w-3xl px-4 pb-16 sm:px-6">
        ...
        <p className="mt-4 text-lg text-slate-300">{phrase.meaningEn}</p>

        <section className="mt-10 space-y-4">
          <h2 className="font-wa-serif text-xl text-slate-100">Nuance</h2>
          <p className="leading-relaxed text-slate-400">{phrase.nuance}</p>
        </section>

        <section className="mt-8 rounded-2xl border border-wa-ruri/25 bg-wa-ruri/8 p-5">
          <h2 className="font-wa-serif text-lg text-slate-100">Cultural note</h2>
          <p className="mt-2 leading-relaxed text-slate-300">{phrase.culturalNote}</p>
        </section>

        <section className="mt-10">
          <h2 className="font-wa-serif text-xl text-slate-100">Examples</h2>
          <ul className="mt-4 space-y-4">
            {phrase.examples.map((ex) => (
              ...
              >
                ...
                <p className="mt-1 text-sm text-slate-400">{ex.en}</p>
                <p className="mt-2 text-[12px] text-slate-500">{ex.context}</p>
              </li>
            ))}
          </ul>
        </section>

        {phrase.commonMistakes.length > 0 ? (
          <section className="mt-10">
            <h2 className="font-wa-serif text-xl text-slate-100">Common mistakes</h2>
        ...
        <div className="mt-12 rounded-2xl border border-pink-500/25 bg-pink-500/8 p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-wa-serif text-lg text-slate-100">Ask Sensei about {phrase.topic}</h2>
        ...
        {related.length > 0 ? (
          <section className="mt-12">
            <h2 className="font-wa-serif text-lg text-slate-100">Related phrases</h2>
